...
(function () {
    ...
    var log = logger.getDBLogger("vn-workorder-put-status-update-js");
    ...
    function execute() {
        ...
        try {
            ...
            var currentTime = STANDARD_DATE_FORMAT.format(System.currentTimeMillis());
            params.put("status", recordData.getRecordData().get("cstWorkTaskStatusTX"));
            params.put("statusChangeTimestamp", currentTime);
            params.put("reason", recordData.getRecordData().get("triCommentTX"));
            ...
            var urlToTrigger = workOrderEndpointURL + "/" + recordData.getRecordData().get("cstExternalWorkOrderIDTX") + "/statusUpdate";
            ...
            log.debug(currentTime + "Sending Status Update : " + recordData.getRecordData().get("cstWorkTaskStatusTX"));
            ...
            if (responseCode >= 200 && responseCode < 300) {
                ...
                recordData.getRecordData().put("cstMessageTX", java.lang.String.format("[%s]:%s | Status updated to %s in Vendor System", responseJSON.code, responseJSON.message, recordData.getRecordData().get("cstWorkTaskStatusTX")));
            } else {
                log.error("Error occurred while posting data");
                if(responseCode != 400){
					recordData.getRecordData().put("cstErrorFlagBL", "true");
					recordData.getRecordData().put("cstRetryTypeLI", "Work Order Status Update");
