...
(function () {
	...
	var log = logger.getDBLogger("vn-vendor-post-workorder-js");
	...
	function execute() {
		...
		try {
			...
			var requestTemplate = {
				...
				"requestedBy": getNSV(recordDataMap.get("cstRequesterTX")),
				"requestedPhone": getNSV(recordDataMap.get("cstPhoneForContactTX")),
				"requestedEmail": getNSV(recordDataMap.get("cstEmailTX")),
				...
				"requestor": {
					...
					"name": getNSV(recordDataMap.get("cstRequesterTX")),
					"email": getNSV(recordDataMap.get("cstEmailTX")),
					"phone": getNSV(recordDataMap.get("cstPhoneForContactTX"))
	...
	*/
			log.info("Payload for workOrder Post JSON : " + JSON.stringify(requestTemplate));
	...
	function fetchRecordData(module, bo, section, fields, recordId) {
		...
		log.info("recordId for lookup : " + recordId);
		...
		log.info("Time to get Data in ms : " + totalTimeToFetch);
